  ...
  await execAndWait(
    ...
    [
      ...
      [
        ...
        'user_id,',
        'timestamp,',
        "COALESCE(json_extract_path_text(properties, 'experiment_id'), json_extract_path_text(properties, 'experimentId')) AS experiment_id,",
        "COALESCE(json_extract_path_text(properties, 'variation_id'), json_extract_path_text(properties, 'variationId')) AS variation_id",
        'FROM experimentation.fact_events',
        "WHERE event_type IN ('experiment_viewed', 'viewed_experiment')",
        'AND user_id IS NOT NULL',
        "AND COALESCE(json_extract_path_text(properties, 'experiment_id'), json_extract_path_text(properties, 'experimentId')) IS NOT NULL",
        "AND COALESCE(json_extract_path_text(properties, 'variation_id'), json_extract_path_text(properties, 'variationId')) IS NOT NULL",
      ].join(' '),
      [
        'CREATE OR REPLACE VIEW experimentation.feature_usage AS',
        'SELECT',
        'timestamp,',
        "COALESCE(json_extract_path_text(properties, 'feature_key'), json_extract_path_text(properties, 'featureKey')) AS feature_key,",
        "COALESCE(json_extract_path_text(properties, 'value'), json_extract_path_text(properties, 'feature_value'), json_extract_path_text(properties, 'featureValue')) AS value",
        'FROM experimentation.fact_events',
        "WHERE event_type IN ('feature_usage', 'feature_evaluated')",
        "AND COALESCE(json_extract_path_text(properties, 'feature_key'), json_extract_path_text(properties, 'featureKey')) IS NOT NULL",
        "AND COALESCE(json_extract_path_text(properties, 'value'), json_extract_path_text(properties, 'feature_value'), json_extract_path_text(properties, 'featureValue')) IS NOT NULL",
      ].join(' '),
    ],
    workgroupName,
    database,
